                ...
                <form class="all-filter" method="get" action="{{lang_url('filter-cat')}}">
                    ...
                    <div class="filter">
                        <div class="form1">
                            ...
                            @foreach($all_sub_cat as $row)
                            <label>   
                                <input type="checkbox" name="sub_cat[]" id="" value="{{ $row->id}}">
                        ...
                        <div class="form2">
                            ...
                            <div class="input-s">
                                ...
                                <input class="float-left" type="text" name="to_price" placeholder="{{trans('cpanel.to')}}">
                                <input class="float-left" type="text" name="from_price" placeholder="{{trans('cpanel.from')}}">
                        ...
                        @if(count($items_colors))
                            <div class="form3">
                                ...
                                <label>
                                    <input type="checkbox" name="colors[]" value="{{ $items_colors->id }}">
                        ...
                        @if(count($items_sizes))
                            ...
                            <div class="form4">
                                ...
                                @foreach( $items_sizes as $items_size)
                                    <label>
                                        <input type="checkbox" name="sizes[]" id="" value="{{ $items_size->id }}">
                        ...
                        <div class="form5">
                           ...
                           @foreach($all_brands as $row)
                            <label>
                                <input type="checkbox" name="brands[]" id="" value="{{$row->id}}">
                                <span class="lbl-text">  {{ $row->title}}</span>
                            </label>
                            @endforeach
                        </div>
...
<!-- 
                      ...
                      @if(isset($search))
                          ...
                          <input id="search_cat_id" type="hidden" value="{{ $category_id }}">
                      @else
                          <input id="isSearch" type="hidden"  value="categories">
                      @endif
...
//    loadMoreData(page); //initial content load
    $(window).scroll(function () {
        if ($(window).scrollTop() + $(window).height() >= $(document).height()) {
            page++;
            console.log("before loaded more data, yahia");
            loadMoreData(page, filter_cat_type);
            console.log("after loaded more data successfully, yahia");
        }
    });


    var load_more_url = '{{lang_url(!isset($search)?"categories"."/".$data['cat_id'] : "search")}}';
//    alert(load_more_url);
    function loadMoreData(page, filter_cat_type = 'menu_all') {
//        alert('sss');
        var dumy_var = $("#isSearch").val();
        var completre_url ="" ;
        if (dumy_var != "categories") {
            completre_url = '?page=' + page + "&category_id=" + $("#search_cat_id").val() +"&search=" + dumy_var  ;
        }else {
            completre_url = '?page=' + page + '&filter_cat_type=' + filter_cat_type ;

        }
    $.ajax({
    url: load_more_url + completre_url,
            type: "get",
            asynk:false,
            cache: false,
            beforeSend: function (){
